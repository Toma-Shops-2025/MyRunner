...
export async function dispatchOrderInternal(orderId: string): Promise<{ ok: boolean; reason?: string }> {
  ...
  if (!candidates || candidates.length === 0) {
    // No one online ù leave queued, cron will re-try
    await supabaseAdmin
      .from("orders")
...
export const setDriverPresence = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Status first ù never block going online if location columns are missing on live DB
...
/** Claim an open paid order ù service role so live RLS cannot silently no-op. */
