...
export const Route = createFileRoute("/signup")({
  validateSearch: (search: Record<string, unknown>) => ({
    ...
    ref: typeof search.ref === "string" ? search.ref.trim() : undefined,
  }),
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Sign up for MyRunner to send deliveries or apply to drive as a Runner.",
      },
...
function Signup() {
  const { intent: initialIntent, ref: refCode } = Route.useSearch();
  const nav = useNavigate();
  const [intent, setIntent] = useState<SignupIntent>(refCode ? "driver" : initialIntent);
  ...
  async function google() {
    ...
    if (refCode) setDriverReferralCode(refCode);
        ...
        >
          ...
          {refCode && (
            <p className="mt-3 rounded-lg border border-gold/30 bg-gold/10 px-3 py-2 text-xs text-gold">
              Referral code applied: <span className="font-mono font-semibold">{refCode.toUpperCase()}</span> — earn $25 after 3 deliveries.
            </p>
          )}
          ...
          <div className="mt-6 grid grid-cols-2 gap-2 rounded-xl border border-border bg-muted/30 p-1">
            <button
              ...
              className={`rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
                !isDriver ? "bg-gold text-primary-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
              }`}
            >
              Send deliveries
            </button>
          ...
          <div className="mt-6 space-y-4">
            <Button type="button" variant="outline" className="w-full" disabled={googleBusy} onClick={google}>
